...
- **Cliente Supabase no Next.js App Router**: padrão oficial `@supabase/ssr` — `createBrowserClient` para componentes client, `createServerClient` para Server Components/Route Handlers, `proxy.ts` (antigo `middleware.ts` — renomeado pelo Next.js 16) para refresh de sessão em cada request.
...
- **Segredos**: a Secret Key (sucessora da `service_role key`) nunca entra no bundle do frontend — o app roda inteiramente com `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (padrão atual do Supabase, formato `sb_publishable_...`; substitui a legada "anon key" JWT — é uma chave pública, protegida por RLS). Chaves do TMDB e Google Books ficam em variáveis **sem** prefixo `NEXT_PUBLIC_`, usadas só em Route Handlers server-side.
